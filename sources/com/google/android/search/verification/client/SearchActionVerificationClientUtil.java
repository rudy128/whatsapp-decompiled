...
public abstract class SearchActionVerificationClientUtil {
    ...
    public static boolean isPackageGoogleSigned(Context context, String str) {
        try {
            ...
            A1a[0] = str;
            AnonymousClass8BR.A1L(A1a, 1, equals);
            AnonymousClass8BR.A1L(A1a, 2, equals2);
            Log.d(TAG, String.format("Package %s GSA signed status %s AssistantGo signed status %s", A1a));
            if (equals || equals2) {
                return true;
            }
            return false;
            ...
            Log.d(TAG, AnonymousClass001.A1H("Unexpected NameNotFoundException: ", valueOf, new StringBuilder(String.valueOf(valueOf).length() + 34)));
            return false;
        }
    }
    ...
    public static void logIntentWithExtras(Intent intent) {
        ...
        Log.d(TAG, AnonymousClass001.A1H("\t", valueOf, new StringBuilder(String.valueOf(valueOf).length() + 1)));
        ...
        if (extras != null) {
            ...
            while (it.hasNext()) {
                ...
                Object[] A1a = C17890vO.A1a(A0v);
                A1a[1] = extras.get(A0v);
    ...
    public static boolean isPackageWhitelisted(Context context, boolean z) {
        ...
        if (z && PublicKeyCredentialControllerUtility.JSON_KEY_USER.equals(Build.TYPE) && !isPackageGoogleSigned(context, nameForUid)) {
            str = "Package is not Google signed!";
            ...
            if (valueOf.length() != 0) {
                str = "Access is not allowed for package: ".concat(valueOf);
            } else {
                str = new String("Access is not allowed for package: ");
            }
        }
        Log.d(TAG, str);
        return false;
